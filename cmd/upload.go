...
var (
	uploadInventory   string // 主机列表（文件路径、目录路径或逗号分隔的主机列表）
	uploadGroup       string
	uploadUser        string
...
示例:
  ...
  # 从文件读取主机列表上传文件
  gossh upload -i hosts.txt -u root -k ~/.ssh/id_rsa -l app.tar.gz -r /tmp/app.tar.gz

  # 从目录读取所有 Ansible hosts 文件并聚合
  gossh upload -i ansible_hosts -u root -k ~/.ssh/id_rsa -l app.tar.gz -r /tmp/app.tar.gz

  # 从命令行参数指定主机上传文件（逗号分隔）
  gossh upload -i "192.168.1.10,192.168.1.11" -u root -l app.tar.gz -r /tmp/app.tar.gz

  # 指定文件权限
  gossh upload -i hosts.txt -u root -l script.sh -r /tmp/script.sh --mode 0755

  # 指定并发数
  gossh upload -i hosts.txt -u root -l app.tar.gz -r /tmp/app.tar.gz --concurrency 10`,
...
func init() {
	...
	// 主机列表相关参数
	uploadCmd.Flags().StringVarP(&uploadInventory, "inventory", "i", "", "主机列表（文件路径、目录路径或逗号分隔的主机列表）。如果指定目录，会递归读取目录下所有子文件并聚合，例如: -i hosts.ini 或 -i hosts_dir/ 或 -i 192.168.1.10,192.168.1.11")
	uploadCmd.Flags().StringVarP(&uploadGroup, "group", "g", "", "Ansible INI 格式的分组名称（仅在使用 -i 参数指定文件或目录时有效），例如: -g test 或 -g web_servers")
